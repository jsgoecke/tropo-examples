...
answer
sleep 2
say 'Welcome to the Ruby Yahoo weather reader', :voice => 'allison'
...
options = { :choices     => "[5 DIGITS]",
            ...
            :onChoice    => lambda { |choice|
              ...
              say weather_results["description"], :voice => 'allison'
              say "The wind chill is #{weather_results["wind"]["chill"]} degrees, " + 
                  "the wind speed is #{weather_results["wind"]["speed"]}", :voice => 'allison'
              say "The forecast is #{weather_results["item"]["forecast"][0]["text"]}, " +
                  "with a high of #{weather_results["item"]["forecast"][0]["high"]} degrees, " +
                  "and a low of #{weather_results["item"]["forecast"][0]["low"]} degrees.", :voice => 'allison'
                }
          }

ask 'Enter the ZIP code for a weather check', options
say 'Thank you for using the Yahoo Weather Service. Goodbye.', :voice => 'allison'
hangup